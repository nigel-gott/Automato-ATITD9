...
function setupGlobals(config)
  NORTH = VectorVegJanitor:new { 0, -1 }
  SOUTH = VectorVegJanitor:new { 0, 1 }
  WEST = VectorVegJanitor:new { -1, 0 }
  EAST = VectorVegJanitor:new { 1, 0 }
  ...
  MOVE_BTNS = {
    [NORTH] = VectorVegJanitor:new { 48, 30 },
    [SOUTH] = VectorVegJanitor:new { 48, 71 },
    [NORTH_EAST] = VectorVegJanitor:new { 70, 30 },
    [WEST] = VectorVegJanitor:new { 30, 50 },
    [EAST] = VectorVegJanitor:new { 70, 50 },
    [NORTH_WEST] = VectorVegJanitor:new { 30, 30 },
    [SOUTH_EAST] = VectorVegJanitor:new { 70, 67 },
    [SOUTH_WEST] = VectorVegJanitor:new { 30, 67 }
  }
...
function PlantLocation:move()
  ...
  for step = 1, self.num_move_steps do
    click(self.move_btn + VectorVegJanitor:new { build_arrows[0], build_arrows[1] }, false, false)
  end
end
...
function displayBox(box, forever, time)
  ...
  moveMouse(VectorVegJanitor:new { box.left, box.top })
...
function compareAnchorPoints(config, plants)
  ...
  for i = 1, 5 do
    ...
    local buildButton = clickPlantButton(config.seed_name)
    local loc = PlantLocation:new { direction_vector = DIRECTIONS["NORTH"], num_move_steps = 5 }
    loc:move()
    lsSleep(100)
    differ:snapshot()
    local pixel_connector = differ:connector()
    pixel_connector:connect()
    assert(pixel_connector:max_label().pixels.size > 0)
    local shape = Shape(pixel_connector:max_label().pixels)
    print("Shape " .. i .. " w:" .. shape.pixels.width .. " h:" .. shape.pixels.height .. " anchor:" .. tostring(shape.anchor_point) .. " adj:" ..
      tostring(shape.anchor_point-Point2D(shape.pixels.min_x, shape.pixels.min_y, shape.anchor_point.system)))
    ...
    for i = 1, config.num_plants do
      for y = box.top, box.top + box.height, 1 do
        for x = box.left, box.left + box.width do
          ...
          local colour = BLACK
          local inside_player = inside(VectorVegJanitor:new { x, y }, sbox)
...
function indexToWindowPos(index)
  ...
  return VectorVegJanitor:new { x, y }
end
...
function makeSearchBox(direction, seed_type)
  ...
  --
  --    local top_left = offset_mid + direction * 40 - VectorVegJanitor:new { 25, 20 }
